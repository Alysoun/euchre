...
import { trumpCallerSeatHint } from '../utils/trumpCallerLabel';
...
interface SeatLabelsProps {
  ...
  trumpCallerId: GameState['trumpCallerId'];
  trumpCallKind: GameState['trumpCallKind'];
  goAlone: GameState['goAlone'];
}
...
const AnchorRoot = styled.div<{ $x: number; $y: number; $editMode?: boolean; $dimmed?: boolean }>`
  ...
  top: ${(p) => p.$y}px;
  z-index: ${(p) =>
    p.$editMode ? Z_LAYOUT_EDIT_TARGET : p.$dimmed ? Z_SEAT_LABEL_DIMMED : Z_SEAT_LABEL};
...
type SeatLabelItemProps = {
  ...
  trumpCallerHint: string | null;
  callerBadgeIcon: string;
  compact: boolean;
  scale: number;
  anchorX: number;
  anchorY: number;
  ...
  const labelBody = (
    <>
      <PlayerDisplay $isHuman={isHuman} $isActive={isActiveTurn} $compact={compact}>
        ...
        {isTrumpCaller && (
          <CallerBadge title={trumpCallerHint ?? 'Made trump'}>{callerBadgeIcon}</CallerBadge>
        )}
        ...
        <Stats>
          ...
          {isTrumpCaller && trumpCallerHint ? ` · ${trumpCallerHint}` : ''}
        </Stats>
      </PlayerDisplay>
    </>
  );
...
const SeatLabels: React.FC<SeatLabelsProps> = ({
  ...
  const showCaller =
    trumpCallerId !== null &&
    (trumpCallKind === 'orderUp' || trumpCallKind === 'nameTrump');
  const callerHint = showCaller ? trumpCallerSeatHint({ trumpCallKind, goAlone }) : null;
  const {
    layoutEditMode,
      ...
      {visiblePlayers.map((player) => {
        ...
        return (
          <SeatLabelItem
            ...
            isTrumpCaller={showCaller && player.id === trumpCallerId}
            trumpCallerHint={showCaller && player.id === trumpCallerId ? callerHint : null}
            callerBadgeIcon={trumpCallKind === 'orderUp' ? '↑' : '♛'}
            compact={compact}
