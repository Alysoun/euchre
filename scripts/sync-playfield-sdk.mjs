/**
 * Copies the Playfield SDK into euchre/playfield so GitHub Actions and clones
 * do not depend on ../playfield beside the repo.
 *
 * Source order:
 *   1. --from <path>
 *   2. ../playfield (sibling of euchre in Games/)
...
const euchreRoot = path.resolve(__dirname, '..');
...
function parseFromArg() {
  ...
  const sibling = path.resolve(euchreRoot, '..', 'playfield');
  if (fs.existsSync(path.join(sibling, 'package.json'))) return sibling;
  const embedded = path.join(euchreRoot, 'playfield');
...
if (!from) {
  console.error(
    'Playfield SDK not found. Place it at euchre/playfield or Games/playfield, or run:\n' +
      '  node scripts/sync-playfield-sdk.mjs --from "C:/path/to/playfield"'
  );
  process.exit(1);
}

const dest = path.join(euchreRoot, 'playfield');
const copyRoots = [
  'package.json',
  'package-lock.json',
...
console.log('Done. Run npm install in euchre if package.json paths changed.');
