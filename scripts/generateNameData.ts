...
function properCapitalization(name: string): string {
  ...
  const preserveNames = ['de la o', 'de la paz', 'de la y'];
  ...
  for (const { pattern, replacement } of compounds.sort(
...
async function fetchSSANames(): Promise<NameEntry[]> {
  ...
  try {
    ...
    return JSON.parse(data) as NameEntry[];
  } catch (error) {
...
const diverseNames: DiverseNames = {
  ...
  lastNames: ['Wang', 'Li', 'Zhang', 'Chen', 'Liu', 'Tanaka', 'Sato', 'Suzuki', 'Kim', 'Park'],
};
...
generateNameData().catch((error: unknown) => {
